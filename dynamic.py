...
def flische (cer_1, cers, biz_name):
    lst_x = []
    ...
    # a_11 = pd.read_excel('res_11.xlsx')
    # a_12 = pd.read_excel('res_12.xlsx')
    nak_ = pd.read_excel('res_nak.xlsx')
    ...
    # a_11 = rename_col(a_11, '+')
    # a_12 = rename_col(a_12, '$')
    # nak_ = rename_col(nak_, '^')
    ...
    # x_5_all = save_all(a_11)
    # x_6_all = save_all(a_12)
    nak_all = save_all(nak_)
    ...
    concate_tab = concat(x_1[['Код статьи']], x_2[['Код статьи']], x_3[['Код статьи']], x_4[['Код статьи']], x_5[['Код статьи']], x_6[['Код статьи']],  nak_x[['Код статьи']])
    # ,,
    #  , ,
    # x_7[['Код статьи']], x_8[['Код статьи']], x_9[['Код статьи']],
    # x_10[['Код статьи']], x_11[['Код статьи']], x_12[['Код статьи']],
    ...
    x_1_all = x_1_all.drop(x_1_all.columns[cer_1:cers], axis=1)
    y_1_all = y_1_all.drop(y_1_all.columns[cer_1:cers], axis=1)
    z_1_all = z_1_all.drop(z_1_all.columns[cer_1:cers], axis=1)
    x_2_all = x_2_all.drop(x_2_all.columns[cer_1:cers], axis=1)
    y_2_all = y_2_all.drop(y_2_all.columns[cer_1:cers], axis=1)
    z_2_all = z_2_all.drop(z_2_all.columns[cer_1:cers], axis=1)
    # x_3_all = x_3_all.drop(x_3_all.columns[cer_1:cers], axis=1)
    # y_3_all = y_3_all.drop(y_3_all.columns[cer_1:cers], axis=1)
    # z_3_all = z_3_all.drop(z_3_all.columns[cer_1:cers], axis=1)
    # z_4_all = z_4_all.drop(z_4_all.columns[cer_1:cers], axis=1)
    # x_5_all = x_5_all.drop(x_5_all.columns[cer_1:cers], axis=1)
    # x_6_all = x_6_all.drop(x_6_all.columns[cer_1:cers], axis=1)
    nak_all = nak_all.drop(nak_all.columns[cer_1:cers], axis=1)

    concat_alloc = concat_(all_rows, x_1_all.iloc[:, 1:6], y_1_all.iloc[:, 1:6], z_1_all.iloc[:, 1:6], x_2_all.iloc[:, 1:6],
                           y_2_all.iloc[:, 1:6],z_2_all.iloc[:, 1:6],
                            nak_all.iloc[:, 1:6])
    ...
    #
    #
    # x_3_all.iloc[:, 1:6], y_3_all.iloc[:, 1:6], z_3_all.iloc[:, 1:6],
    # z_4_all.iloc[:, 1:6], x_5_all.iloc[:, 1:6], x_6_all.iloc[:, 1:6]
    ...
    x_1 = x_1.drop(x_1.columns[cer_1:cers], axis=1)
    x_2 = x_2.drop(x_2.columns[cer_1:cers], axis=1)
    x_3 = x_3.drop(x_3.columns[cer_1:cers], axis=1)
    x_4 = x_4.drop(x_4.columns[cer_1:cers], axis=1)
    x_5 = x_5.drop(x_5.columns[cer_1:cers], axis=1)
    x_6 = x_6.drop(x_6.columns[cer_1:cers], axis=1)
    # x_7 = x_7.drop(x_7.columns[cer_1:cers], axis=1)
    # x_8 = x_8.drop(x_8.columns[cer_1:cers], axis=1)
    # x_9 = x_9.drop(x_9.columns[cer_1:cers], axis=1)
    # x_10 = x_10.drop(x_10.columns[cer_1:cers], axis=1)
    # x_11 = x_11.drop(x_11.columns[cer_1:cers], axis=1)
    # x_12 = x_12.drop(x_12.columns[cer_1:cers], axis=1)
    nak_x = nak_x.drop(nak_x.columns[cer_1:cers], axis=1)


    finish_data_1 = merge(data, x_1.iloc[:, :6], x_2.iloc[:, :6], x_3.iloc[:, :6], x_4.iloc[:, :6],x_5.iloc[:, :6], x_6.iloc[:, :6],
                          nak_x.iloc[:, :6])

    #
    # , x_7.iloc[:, :6], x_8.iloc[:, :6],
    # x_9.iloc[:, :6], x_10.iloc[:, :6], x_11.iloc[:, :6], x_12.iloc[:, :6],
    ...
    cols = [i for i in range(6, len(finish_data_1.columns), 5)]

    finish_data_1.drop(finish_data_1.columns[cols],axis=1,inplace=True)


    finish_data_1 = finish_data_1.fillna(0)
    res = finish_data_1

    num_col = 2
    while num_col <= len(res.columns)-1:
    ...
    lst_names = [biz_name]*6 + ['Накопичувально']
    ...
    ws.title = biz_name
    wb.save(f"C:\\Users\\admin\PycharmProjects\\fin_b\\files\\{biz_name}_2023.xlsx")
    wb.close()


lst_names = ['Корпоративний бізнес', 'Корпоратив - мережа', 'VIP Корпоратив',
                 'Малий та середній бізнес', 'Роздрібний  банкінг', 'Інші']
lst_numbs = [6,10,14,18,22,26]


flische(6, 30, 'Консолідовано Банк')

for i in range(len(lst_numbs)):
    flische(2, lst_numbs[i], lst_names[i])
    print(f"Час вигрузки задачі", datetime.now() - startTime)
